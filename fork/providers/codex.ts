...
export function runWithCodex(input: string, profile: string): string {
  const result = spawnSync(CLI_NAME, ["exec", input], {
    encoding: "utf-8",
    maxBuffer: 1024 * 1024
  })
  ...
  if (!output.trim()) {
    throw new Error(
      `Codex returned empty output for profile "${profile}". ` +
